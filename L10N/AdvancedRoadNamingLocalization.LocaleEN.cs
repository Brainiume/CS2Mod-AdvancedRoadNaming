        ...
        public sealed class LocaleEN : IDictionarySource
        {
            ...
            public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
            {
                return new Dictionary<string, string>
                {
                    ...
                    { _settings.GetOptionDescLocaleID(nameof(AdvancedRoadNamingSettings.EnableRouteShields)), "Show route shield overlays for saved routes that have a shield style selected." },
                    ...
                    { _settings.GetOptionDescLocaleID(nameof(AdvancedRoadNamingSettings.StatisticsPresetOption)), "Balanced emphasizes representative busy-road performance. Corridor Scale emphasizes total route load. Bottleneck Analysis emphasizes congestion. Custom exposes every calculation control." },
                    ...
                    { _settings.GetOptionDescLocaleID(nameof(AdvancedRoadNamingSettings.StatisticsRefreshRateOption)), "Fast refreshes every 128 simulation frames, Balanced every 256, and Efficient every 512. Calculations remain inactive while the foldout is closed." },
